/*Promijeniti program treće vježbe 3b (monitori) tako da dretve neradna_dretva 
po uzimanju broja y trebaju dinamički zauzeti spremnik veličine y mod 20 (isti otpuštaju nakon "potrošnje").
Simulirati dinamičko upravljanje memorijom korištenjem metode "najbolji odgovarajući" 
(ostvarenjem funkcijama zauzmi+oslobodi).
Veličina spremnika koji se dodijeljuje neka bude proizvoljno zadana, 
ali tako da se vide problemi zbog fragmentacije i nedostatka spremnika.
Kada neka dretva ne može dobiti potrebni dio spremnika, npr. operacija zauzmi(v)
vrati grešku (npr. NULL, 0, -1 i slično), zaustaviti tu dretvu (dodatnim redom uvjeta).
Ponovno ju probuditi kada neka druga dretva oslobodi zauzeti dio spremnika (oslobodi(m)).
Nije neophodno ostvariti pravi algoritam za upravljanje spremnikom. 
Dovoljno je simulirati njegov rad. 
U ispisu programa mora biti vidljivo stanje spremnika. 
Slobodne dijelove označiti s npr. - a zauzete s id-jem dretve 
(samo zadnjom znamenkom - neka sučelja primaju id dretve kao dodatni argument: 
zauzmi(v, id), oslobodi(m, id)). 
Npr. spremnik od 50 mjesta bi mogao u nekom trenutku izgledati ovako: 3333----444444-----222222222222222221111111-------.
Ispisati stanje spremnika nakon svake promjene.
 */
